...
describe('Analytics', function() {
    ...
    describe('#_parseQuery', function() {
        describe('user settings', function() {
            ...
            it('should parse `dda_aid` and set anonymousId', function() {
                sinon.spy(user, 'anonymousId');
                analytics._parseQuery('?dda_aid=123');
                assert(user.anonymousId.calledWith('123'));
            });

            it('should parse `dda_uid` and call identify', function() {
                analytics._parseQuery('?dda_uid=123');
                assert(analytics.identify.calledWith('123', {}));
            });

            it('should include traits in identify', function() {
                analytics._parseQuery('?dda_uid=123&dda_trait_name=chris');
        ...
        describe('events', function() {
            ...
            it('should parse `dda_event` and call track', function() {
                analytics._parseQuery('?dda_event=test');
                assert(analytics.track.calledWith('test', {}));
            });

            it('should include properties in track', function() {
                analytics._parseQuery('?dda_event=Started+Trial&dda_prop_plan=Silver');
    ...
    describe('#addIntegrationMiddleware', function() {
        ...
        it('should throw an error if the selected Middleware is not a function', function() {
            try {
                ...
                assert(e.message === 'attempted to add non-function middleware', 'wrong error return');
            }
        });

        it('should throw an error if dda has already initialized', function() {
            analytics.init();
            try {
                analytics.addIntegrationMiddleware(function() {});
                ...
                assert(e.message === 'attempted to add middleware after initialization', 'wrong error return');
            }
        });
