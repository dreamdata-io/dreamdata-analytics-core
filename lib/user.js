...
var debug = require('debug')('dreamdata:user');
...
User.prototype.anonymousId = function (anonymousId) {
    ...
    // set / remove
    if (arguments.length) {
        store.set('dda_anonymous_id', anonymousId);
        this._setAnonymousIdInLocalStorage(anonymousId);
        return this;
    }

    // new
    anonymousId = store.get('dda_anonymous_id');
    if (anonymousId) {
    ...
    if (!this._options.localStorageFallbackDisabled) {
        ...
        anonymousId = localStorage.get('dda_anonymous_id');
        if (anonymousId) {
    ...
    store.set('dda_anonymous_id', anonymousId);
    this._setAnonymousIdInLocalStorage(anonymousId);
    return store.get('dda_anonymous_id');
};
...
User.prototype._setAnonymousIdInLocalStorage = function (id) {
    if (!this._options.localStorageFallbackDisabled) {
        localStorage.set('dda_anonymous_id', id);
    }
};
